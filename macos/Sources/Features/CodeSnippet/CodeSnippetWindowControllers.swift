...
/// 新增/修改代码片段分类的标准 macOS 模态窗口。
final class CodeSnippetCategoryWindowController: ModalWindowController {
    init(
        category: CodeSnippetCategory? = nil,
        config: Ghostty.Config?,
        parentWindow: NSWindow? = nil,
        onSave: @escaping (CodeSnippetCategory) -> Void,
        ...
        let view = CodeSnippetCategoryView(
            category: category,
            onSave: { category in
                onSave(category)
                self.close()
...
private struct CodeSnippetCategoryView: View {
    let category: CodeSnippetCategory?
    let onSave: (CodeSnippetCategory) -> Void
    let onCancel: () -> Void
    ...
    private func confirm() {
        ...
        var saved = category ?? CodeSnippetCategory(name: trimmed)
        saved.name = trimmed
        onSave(saved)
    }
}
